...
public interface RosieLib extends Library {
    ...
    RosieString.ByReference rosie_new_string_ptr(String msg, int len);

    // void rosie_free_string(str s);
    void rosie_free_string(RosieString.ByValue s);

    // void rosie_free_string_ptr(str *s);
    void rosie_free_string_ptr(RosieString.ByReference s);

    // void *rosie_new(str *errors);
    Pointer rosie_new(RosieString.ByReference errors);
    ...
    int rosie_config(Pointer L, RosieString.ByReference retvals);

    // int rosie_compile(void *L, str *expression, int *pat, str *errors);
    int rosie_compile(Pointer L, RosieString.ByReference expression, IntByReference pat, RosieString.ByReference errors);
    ...
    int rosie_match(Pointer L, int pat, int start, String encoder, RosieString.ByReference input, RosieMatch.ByReference match);
    ...
    int rosie_matchfile(Pointer L, int pat, String encoder, int wholefileflag,
                        ...
                        RosieString.ByReference err);

    // int rosie_trace(void *L, int pat, int start, char *trace_style, str *input, int *matched, str *trace);
    int rosie_trace(Pointer L, int pat, int start, String trace_style, RosieString.ByReference input,
                    IntByReference matched, RosieString.ByReference trace);

    // int rosie_load(void *L, int *ok, str *src, str *pkgname, str *errors);
    int rosie_load(Pointer L, IntByReference ok, RosieString.ByReference src, RosieString.ByReference pkgname, RosieString.ByReference errors);

    // int rosie_import(void *L, int *ok, str *pkgname, str *as, str *errors);
    int rosie_import(Pointer L, IntByReference ok, RosieString.ByReference pkgname, RosieString.ByReference as, RosieString.ByReference errors);
}
